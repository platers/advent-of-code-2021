...
fn eval(input: u128) -> (i128, usize) {
    let mut z = 0;
    let mut matches = 0;
    let mut digits = input.to_string().chars().map(|c| c.to_digit(10).unwrap() as i128).collect::<Vec<i128>>();
...
fn main() {
    let mut i = 21111111111111;
    let mut min_val = i as i128;
    loop {
        // if there are zeros, incremment them
        let digits = i.to_string().chars().map(|c| c.to_digit(10).unwrap() as i128).collect::<Vec<i128>>();
